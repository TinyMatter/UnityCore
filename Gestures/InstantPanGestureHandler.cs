...
namespace TinyMatter.Core.Gestures {
    public class InstantPanGestureHandler : GestureHandler {
        
        [SerializeField] private int minimumNumberOfTouchesToTrack = 1;
    
        [SerializeField] private float maxDistance = float.MaxValue;
    
        private readonly PressGestureRecognizer gesture = new PressGestureRecognizer();
    
        private void Start() {
            FingersScript.Instance.AddGesture(gesture);
            gesture.MinimumNumberOfTouchesToTrack = minimumNumberOfTouchesToTrack;
            gesture.ThresholdUnits = maxDistance;
        ...
        [NonSerialized] public List<Vector3> registeredPoints = new List<Vector3>();
